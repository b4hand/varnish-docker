...
RUN curl https://repo.varnish-cache.org/GPG-key.txt | apt-key add - \
  ...
  && apt-get update && apt-get install -y varnish && rm /etc/init.d/varnish* /etc/logrotate.d/varnish && chown varnish:adm /var/log/varnish/
...
COPY logrotate.conf /etc/logrotate.d/varnish
